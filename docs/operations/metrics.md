...
- **Claim size is not a metric.** The size of the `groups` claim (the
  operational risk: token/header bloat, see
  [claim-size guidance](../reference/configuration.md#claim-size-guidance))
  is only observable via the `groups_count` field on the per-request log
  lines. Until a histogram exists, a log-based alert (e.g. Loki/CloudWatch
  on `groups_count > N`) is the available proxy.
- **Batch sync outcome** (`users_processed`, grants added/updated/removed) is
  reported in the sync JSON output and logs, not as metrics; the `sync`
  subcommand exits before any scrape could observe them.
